...
async function saveShortLink(payload) {
  ...
  const { error } = await sb.from('diagrams').insert({ id, title, payload });
...
// Load diagram from Supabase by short ID
async function loadShortLink(id) {
  const sb = getSB();
  if (!sb) return null;
  const { data, error } = await sb.from('diagrams').select('payload,title').eq('id', id).single();
  if (error || !data) return null;
  // Fire view count increment (best-effort, non-blocking)
